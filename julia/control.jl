#==============================================================================
 ...
 = Last Change: 2016 Set 25 00:34:49
...
#=
 ...
 =#
function hl_motion_control(i, Ai, Hi, Di, Si, T, TI, n_bot, n_ref, x, v, r_com, r_cov, h, p, gamma, phi, RSSI_SENS, vxl, val)
    ...
    for j in N2
        ...
        Hixy = Hixy + T'*h*h*psi_ij*gamma[1]*T + phi_ij*gamma[2] + (psi_ij - Hi[j])*gamma[3] + T'*h*h*psi_ij*gamma[7]*T
        Hitheta = Hitheta + T'*h*h*psi_ij*gamma[1]*T
        fix = fix + (Xi[:, 1] - Xd[:, 1])'*h*psi_ij*gamma[1]*T + Vj[:, 1]'*phi_ij*gamma[2] - Vj[:, 1]'*(psi_ij - Hi[j])*gamma[3] - Sij'*h*psi_ij*gamma[7]*T
        fiy = fiy + (Xi[:, 2] - Xd[:, 2])'*h*psi_ij*gamma[1]*T + Vj[:, 2]'*phi_ij*gamma[2] - Vj[:, 2]'*(psi_ij - Hi[j])*gamma[3] - Sij'*h*psi_ij*gamma[7]*T
    ...
    if n_ref > 0
        for r = n_bot + 1 : n_bot + n_ref
            ...
            Hixy = Hixy + T'*h*h*Ai[r]*gamma[4]*T + Ai[r]*gamma[5]
