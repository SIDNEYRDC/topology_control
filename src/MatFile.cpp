...
bool MatFile::Open2Read(const string& fn) {
    if(file >= 0) Close();
...
bool MatFile::WriteCube(const string& varname, int nx, int ny, int nz, const double* data) {
    if(file < 0)
        return false;
...
bool MatFile::PrependHeader(const char* filename) {
    ...
    // Get file length.
    fseek(fd, 0, SEEK_END);
    unsigned long long length = ftell(fd);
    fseek(fd, 0, SEEK_SET);
